...
if image:
    ...
    size.markdown(f"<h5>Size :{img.size}</h5>",unsafe_allow_html=True)
    mode.markdown(f"<h5>Mode :{img.mode}</h5>",unsafe_allow_html=True)
    format_.markdown(f"<h5>Format :{img.format}</h5>",unsafe_allow_html=True)
    st.markdown("<h2 style='text-align: center;'>Resizing</h2>",unsafe_allow_html=True)
    # take input from user for width and height
    ...
    # use pillow doc official for filter feature
    ...
    if s_btn:
        ...
        # st.image(edited)
        filtered=edited
        # for filter first import form PIL.ImageFilter import *
        if filters != "None":
            if filters =="Blur":
